  ...
  <AppShell>
    ...
    <template v-else>
      ...
      <div class="bg-white border border-slate-200 rounded-xl p-5">
        ...
        <p class="text-xs text-slate-500 mb-4">De quién recibió el material y de lo entregado. Puedes subir varias de cada tipo.</p>
        ...
        <div v-for="grupo in gruposFoto" :key="grupo.tipo" class="mb-5 last:mb-0">
          <div class="flex items-center justify-between mb-2">
            ...
            <label class="min-h-[30px] flex items-center bg-primary text-white text-[13px] font-bold rounded-lg px-3 cursor-pointer" :class="{ 'opacity-50 pointer-events-none': subiendo === grupo.tipo }">
...
const gruposFoto = [
  { tipo: 'personal', label: 'Personal que recibió' },
  { tipo: 'material', label: 'Material entregado' },
];

function fotosPorTipo(tipo) {
  return (salida.value?.fotos || []).filter((f) => f.tipo === tipo);
}

async function cargarMiniatura(f) {
  if (miniaturas[f.id]) return;
  try {
    const { data } = await api.get(`/salidas-almacen/${route.params.id}/fotos/${f.id}`, { responseType: 'blob' });
...
async function subirFoto(ev, tipo) {
  ...
  try {
    ...
    await api.post(`/salidas-almacen/${route.params.id}/fotos`, form);
    await cargar();
  } catch (err) {
...
async function eliminarFoto(f) {
  ...
  try {
    await api.delete(`/salidas-almacen/${route.params.id}/fotos/${f.id}`);
